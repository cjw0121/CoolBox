...
var Cookie = {
		setCookie: function(key, value, dayToLive) {	
			...
			document.cookie = key + '=' + encodeURIComponent(value) + ( (dayToLive === null) ? '' : '; expires=' +exp.toGMTString() ) + ";path=/;domain=shop.com";
		},
...
$(function() {
		...
		if (!layout.judgeEmpty([logInObj.name, logInObj.psw], '填写信息不能为空！', 'warn-text')) {
			...
			layout.ajaxPost(logInObj.form.attr('action'), sendData, function(data, status) {
				...
				if (data.result == 'success') {
					...
					//跳转页面
					layout.backHome(2, $('#nav-option li a:first').attr('href'), '登录成功', 'success-text');					
				} else {
					layout.msgHandle.prompt(data.error, 'warn-text');
				}
			});
		}
	});
		...
		if (!layout.judgeEmpty([registerObj.name, registerObj.psw, registerObj.email], '填写信息不能为空！', 'warn-text')) {
			...
			layout.ajaxPost(registerObj.form.attr('action'), sendData, function(data, status) {
				...
				if (data.result == 'success') {
					...
					//页面跳转
					layout.backHome(2, $('#nav-option li a:first').attr('href'), '注册成功', 'success-text');
				} else {
	...
	if (!!Cookie.getCookie('login')) {
		...
		$navOption.html('<li id="major-option"><span id="user-name">' + user + '</span><figure><img src="' + pic + '" /></figure><ul><li><a href="/coolbox/coolbox/Home/Index/index">主页</a></li><li><a href="/coolbox/coolbox/Home/User/center?userid=' + id + '">个人空间</a></li><li><a href="/coolbox/coolbox/Home/Index/about">关于</a></li><li id="logout-btn">注销</li></ul></li>');
